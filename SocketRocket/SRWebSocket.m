...
#define CFStreamPropertyWriteTimeout CFSTR("_kCFStreamPropertyWriteTimeout")

typedef enum
{
  SRIdleTimerStateNone,
  SRIdleTimerStateConnection,
  SRIdleTimerStateRead,
  SRIdleTimerStatePing,
} SRIdleTimerState;

NSString *SRIdleTimerStateToString(SRIdleTimerState state)
{
  ...
  {
    states = @{
      @(SRIdleTimerStateNone) : @"None",
      @(SRIdleTimerStateConnection) : @"Connection",
      @(SRIdleTimerStateRead) : @"Read",
      @(SRIdleTimerStatePing) : @"Ping",
    };
  });
  return states[@(state)];
}
...
#if DEBUG
#define SRFastLog(...) NSLog(__VA_ARGS__)
#else
#define SRFastLog(...)
#endif

NSString *const SRWebSocketAppendToSecKeyString = @"258EAFA5-E914-47DA-95CA-C5AB0DC85B11";
NSString *const SRWebSocketErrorDomain = @"SRWebSocketErrorDomain";
NSString *const SRWebSocketPinnedCertificatesKey = @"SR_SSLPinnedCertificates";
...
static NSString *Base64SHA1String(const char *bytes, size_t length)
{
...
- (NSString *)stringBySHA1ThenBase64Encoding
{
  return Base64SHA1String(self.bytes, self.length);
}
...
- (NSString *)stringBySHA1ThenBase64Encoding
{
  return Base64SHA1String(self.UTF8String, self.length);
}
...
@implementation SRWebSocket
{
  ...
  NSTimer *_idleTimer;
  SRIdleTimerState _idleTimerState;
}
...
{
  _writeTimeout = 20.0;
  _idleTimeoutInterval = 20.0;
  _idleTimerState = SRIdleTimerStateNone;
  _useIdleTimeout = NO;
...
{
  assert(_url);
  NSAssert(_readyState == SR_CONNECTING, @"[SR] Cannot call -(void)open on SRWebSocket more than once");

  _selfRetain = self;

  [self startIdleTimerWithState:SRIdleTimerStateConnection];
...
- (void)_HTTPHeadersDidFinish
{
  ...
  if(responseCode >= 400) {
    SRFastLog(@"[SR] Request failed with response code %d", (int)responseCode);
    [self _failWithError:[NSError errorWithDomain:SRWebSocketErrorDomain code:2132 userInfo:@{ NSLocalizedDescriptionKey : [NSString stringWithFormat:@"Received bad response code from server: %ld", (long)responseCode] }]];
    return;
  }
  ...
  [self stopIdleTimer];
  [self startIdleTimerWithState:SRIdleTimerStateRead];
...
- (void)_readHTTPHeader
{
  ...
  [self _readUntilHeaderCompleteWithCallback:^(SRWebSocket *webSocket, NSData *data)
  {
    ...
    if(CFHTTPMessageIsHeaderComplete(_receivedHTTPHeaders)) {
      SRFastLog(@"[SR] Finished reading headers %@", CFBridgingRelease(CFHTTPMessageCopyAllHeaderFields(_receivedHTTPHeaders)));
...
- (void)didConnect
{
  ...
  _secKey = [keyBytes respondsToSelector:@selector(base64EncodedStringWithOptions:)] ? [keyBytes base64EncodedStringWithOptions:0] : [keyBytes base64Encoding];
  assert(_secKey.length == 24);

  CFHTTPMessageSetHeaderFieldValue(request, CFSTR("Upgrade"), CFSTR("websocket"));
  CFHTTPMessageSetHeaderFieldValue(request, CFSTR("Connection"), CFSTR("Upgrade"));
  CFHTTPMessageSetHeaderFieldValue(request, CFSTR("Sec-WebSocket-Key"), (__bridge CFStringRef)_secKey);
  CFHTTPMessageSetHeaderFieldValue(request, CFSTR("Sec-WebSocket-Version"), (__bridge CFStringRef)[NSString stringWithFormat:@"%d", (int)_webSocketVersion]);
...
- (void)_initializeStreams
{
  ...
  [_outputStream setProperty:@(self.writeTimeout) forKey:(__bridge NSString *)CFStreamPropertyWriteTimeout];
...
#if DEBUG
    [SSLOptions setValue:@(NO) forKey:(__bridge id)kCFStreamSSLValidatesCertificateChain];
    SRFastLog(@"[SR] In debug mode. Allowing connection to any root cert");
#endif
...
- (void)closeWithCode:(NSInteger)code reason:(NSString *)reason
{
  ...
  dispatch_async(_workQueue, ^
  {
    ...
    SRFastLog(@"[SR] Closing with code %d reason %@", (int)code, reason);
    ...
    if(reason) {
      ...
      BOOL success = [reason getBytes:(char *)mutablePayload.mutableBytes + sizeof(uint16_t) maxLength:payload.length - sizeof(uint16_t) usedLength:&usedLength
        encoding:NSUTF8StringEncoding options:NSStringEncodingConversionExternalRepresentation range:NSMakeRange(0, reason.length) remainingRange:&remainingRange];
...
- (void)_failWithError:(NSError *)error
{
  dispatch_async(_workQueue, ^
  {
    ...
    if(self.readyState != SR_CLOSED) {
      ...
      SRFastLog(@"[SR] Failing with error %@", error.localizedDescription);
...
- (void)send:(id)data
{
  NSAssert(self.readyState == SR_OPEN, @"[SR] Invalid State: Cannot call send: until connection is open");
  SRFastLog(@"[SR] Send: %@", data);
...
{
  NSAssert(self.readyState == SR_OPEN, @"[SR] Invalid State: Cannot call send: until connection is open");
  SRFastLog(@"[SR] Send ping");

  dispatch_async(_workQueue, ^
  {
    [self startIdleTimerWithState:SRIdleTimerStatePing];
...
{
  SRFastLog(@"[SR] Send pong with data: %@", data);
...
{
  SRFastLog(@"[SR] Received ping with data: %@", pingData);

  // Need to ping-pong this off _callbackQueue first to make sure messages happen in order
  [self _performDelegateBlock:^
  {
    [self sendPongWithData:pingData];
  }];
}

- (void)handlePong
{
  SRFastLog(@"[SR] Received pong");
...
static inline BOOL closeCodeIsValid(int closeCode)
{
  ...
  if(closeCode >= 1000 && closeCode <= 1011)
    return (closeCode == 1004 || closeCode == 1005 || closeCode == 1006) ? NO : YES;
...
- (void)handleCloseWithData:(NSData *)data
{
  ...
  SRFastLog(@"[SR] Received close frame");
...
- (void)_disconnect
{
  ...
  SRFastLog(@"[SR] Trying to disconnect");
...
- (void)_readFrameContinue
{
  ...
  [self _addConsumerWithDataLength:2 callback:^(SRWebSocket *webSocket, NSData *data)
  {
    ...
    header.fin = (SRFinMask & headerBuffer[0]) != 0;
    header.masked = (SRMaskMask & headerBuffer[1]) != 0;
    ...
    if(extra_bytes_needed == 0)
      ...
      [webSocket _addConsumerWithDataLength:extra_bytes_needed callback:^(SRWebSocket *ws, NSData *dt)
      {
        size_t mapped_size = dt.length;
        const void *mapped_buffer = dt.bytes;
        size_t offset = 0;
        ...
        if(header.masked) {
          ...
          memcpy(ws->_currentReadMaskKey, ((uint8_t *)mapped_buffer) + offset, sizeof(ws->_currentReadMaskKey));
        }

        [ws _handleFrameHeader:header curData:ws->_currentFrameData];
...
- (void)_sendFrameWithOpcode:(SROpCode)opcode data:(id)data
{
  ...
  if(!frame) {
    [self closeWithCode:SRStatusCodeMessageTooBig reason:@"Event too big"];
    return;
  }
  ...
  dispatch_async(_workQueue, ^
  {
    switch(eventCode) {
      case NSStreamEventOpenCompleted: {
        SRFastLog(@"[SR] NSStreamEventOpenCompleted");

        if(_readyState >= SR_CLOSING)
          return;

        assert(_readBuffer);

        if(_readyState == SR_CONNECTING && aStream == _inputStream)
          [self didConnect];
      ...
      case NSStreamEventErrorOccurred: {
        SRFastLog(@"[SR] NSStreamEventErrorOccurred %@", aStream.streamError);
      ...
      case NSStreamEventEndEncountered: {
        SRFastLog(@"[SR] NSStreamEventEndEncountered");
      ...
      case NSStreamEventHasBytesAvailable: {
        SRFastLog(@"[SR] NSStreamEventHasBytesAvailable");
      ...
      case NSStreamEventHasSpaceAvailable: {
        SRFastLog(@"[SR] NSStreamEventHasSpaceAvailable");
      ...
      default:
        SRFastLog(@"[SR] NSStreamEvent (default)");
        break;
    }
  });
}
...
- (void)startIdleTimerWithState:(SRIdleTimerState)state
{
  if(!_useIdleTimeout || _idleTimerState == SRIdleTimerStateConnection || _idleTimerState == SRIdleTimerStatePing)
    return;
  ...
  SRFastLog(@"[SR] Idle timer started with state: %@", SRIdleTimerStateToString(state));

  _idleTimerState = state;

  _idleTimer = [NSTimer timerWithTimeInterval:_idleTimeoutInterval target:self selector:@selector(idleTimerEvent) userInfo:nil repeats:NO];
  [[NSRunLoop SR_networkRunLoop] addTimer:_idleTimer forMode:NSDefaultRunLoopMode];
}

- (void)stopIdleTimer
{
  if(_idleTimer) {
    SRFastLog(@"[SR] Idle timer stopped with state: %@", SRIdleTimerStateToString(_idleTimerState));
...
- (void)idleTimerEvent
{
  SRFastLog(@"[SR] Idle timer fired with state: %@", SRIdleTimerStateToString(_idleTimerState));

  SRIdleTimerState state = _idleTimerState;

  [self stopIdleTimer];

  switch(state) {
    case SRIdleTimerStateNone:
      break;
    case SRIdleTimerStateRead:
      if(_readyState == SR_OPEN)
        [self sendPing];
      break;
    case SRIdleTimerStateConnection:
    case SRIdleTimerStatePing:
      [self _failWithError:[NSError errorWithDomain:SRWebSocketErrorDomain code:408 userInfo:@{ NSLocalizedDescriptionKey : @"Connection timed out" }]];
      break;
  }
}
...
{
  return [NSURLProtocol propertyForKey:SRWebSocketPinnedCertificatesKey inRequest:self];
}
...
{
  [NSURLProtocol setProperty:SR_SSLPinnedCertificates forKey:SRWebSocketPinnedCertificatesKey inRequest:self];
}
